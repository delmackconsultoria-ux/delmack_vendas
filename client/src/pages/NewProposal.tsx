...
import { ArrowLeft, Save, Search, CheckCircle, Loader, CheckCircle2, AlertCircle, Upload, FileText, Calculator, XCircle, MapPin, ExternalLink } from "lucide-react";
...
import { getProperfyFieldClassName, getProperfyFieldStyle } from "@/lib/properfyFieldHelper";
...
interface FormData {
  ...
  brokerAngariadorRealEstateCompany?: string;
  brokerAngariadorRealEstateCnpj?: string;
...
export default function NewProposal() {
  ...
  // Buscar URL do documento já enviado (modo edição)
  const { data: existingDocUrl } = trpc.sales.getProposalDocument.useQuery(
    { saleId: editId || "" },
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    brokerAngariadorRealEstateCompany: "",
    brokerAngariadorRealEstateCnpj: "",
  ...
  const commissionCalc = useMemo(() => {
    ...
    const saleValue = parseCurrencyInput(formData.saleValue);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Validar arquivo
    const validation = validateFile(file, FILE_VALIDATION_CONFIG.PROPOSAL_DOCUMENT);
  ...
  // Validação de comprovante de sinal
  const handleSinalComprovanteChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
  ...
  useEffect(() => {
    if (isEditMode && existingSale) {
      const s = existingSale as any;
      setFormData(prev => ({
        ...prev,
        // Imóvel
        propertyType: s.propertyType || "baggio",
        propertyReference: s.propertyReference || "",
        propertyAddress: s.property?.address || s.propertyAddress || "",
        propertyNumber: s.propertyNumber || "",
        propertyComplement: s.propertyComplement || "",
        propertyNeighborhood: s.property?.neighborhood || s.propertyNeighborhood || "",
        propertyCity: s.property?.city || s.propertyCity || "",
        propertyState: s.property?.state || s.propertyState || "",
        propertyZipCode: s.property?.zipCode || s.propertyZipCode || "",
        advertisementValue: s.advertisementValue?.toString() || "",
        condominiumName: s.condominiumName || "",
        // Detalhes do imóvel
        typeOfProperty: s.propertyType || s.typeOfProperty || "",
        bedrooms: s.bedrooms?.toString() || "",
        costPerM2: s.costPerM2?.toString() || "",
        privateArea: s.privateArea?.toString() || "",
        totalArea: s.totalArea?.toString() || "",
        propertyAge: s.propertyAge?.toString() || "",
        // Venda
        saleDate: s.saleDate ? new Date(s.saleDate).toISOString().split('T')[0] : "",
        angariationDate: s.angariationDate ? new Date(s.angariationDate).toISOString().split('T')[0] : "",
        saleValue: s.saleValue?.toString() || "",
        expectedPaymentDate: s.expectedPaymentDate ? new Date(s.expectedPaymentDate).toISOString().split('T')[0] : "",
        saleType: s.saleType || "",
        responsible: s.responsible || "",
        invoiceNumber: s.invoiceNumber || "",
        // Comprador
        buyerName: s.buyerName || "",
        buyerCpfCnpj: s.buyerCpfCnpj || "",
        buyerPhone: s.buyerPhone || "",
        clientOrigin: s.clientOrigin || "",
        paymentMethod: s.paymentMethod || "",
        financedValue: s.financedValue?.toString() || "",
        // Vendedor
        sellerName: s.sellerName || "",
        sellerCpfCnpj: s.sellerCpfCnpj || "",
        sellerPhone: s.sellerPhone || "",
        // Preencher arrays para o formulário
        buyers: [{ name: s.buyerName || "", cpfCnpj: s.buyerCpfCnpj || "", phone: s.buyerPhone || "", origin: s.clientOrigin || "" }],
        sellers: [{ name: s.sellerName || "", cpfCnpj: s.sellerCpfCnpj || "", phone: s.sellerPhone || "" }],
        // Informações adicionais
        cartoryBank: s.cartoryBank || "",
        despachante: s.despachante || "",
        investmentType: s.investmentType || "",
        bankName: s.bankName || "",
        // Corretores
        brokerAngariadorType: s.brokerAngariadorType || "internal",
        brokerAngariador: s.brokerAngariador || "",
        brokerAngariadorName: s.brokerAngariadorName || "",
        brokerAngariadorCreci: s.brokerAngariadorCreci || "",
        brokerAngariadorEmail: s.brokerAngariadorEmail || "",
        brokerAngariadorRealEstateCompany: s.brokerAngariadorRealEstateCompany || "",
        brokerAngariadorRealEstateCnpj: s.brokerAngariadorRealEstateCnpj || "",
        brokerVendedorType: s.brokerVendedorType || "internal",
        brokerVendedor: s.brokerVendedor || "",
        brokerVendedorName: s.brokerVendedorName || "",
        brokerVendedorCreci: s.brokerVendedorCreci || "",
        brokerVendedorEmail: s.brokerVendedorEmail || "",
        businessType: s.businessType || "",
        // Comissões (sistema automático)
        tipoComissao: s.tipoComissao || "",
        porcentagemComissao: s.porcentagemComissao?.toString() || "",
        comissaoTotal: s.comissaoTotal?.toString() || "",
        comissaoAngariador: s.comissaoAngariador?.toString() || "",
        comissaoCoordenador: s.comissaoCoordenador?.toString() || "",
        comissaoVendedor: s.comissaoVendedor?.toString() || "",
        comissaoImobiliaria: s.comissaoImobiliaria?.toString() || "",
        comissaoParceira: s.comissaoParceira?.toString() || "",
        comissaoAutonomo: s.comissaoAutonomo?.toString() || "",
        // Comissões (campos antigos - compatibilidade)
        totalCommissionPercent: s.totalCommissionPercent?.toString() || "",
        totalCommissionValue: s.totalCommission?.toString() || "",
        angariadorCommission: s.angariadorCommission?.toString() || "",
        vendedorCommission: s.vendedorCommission?.toString() || "",
        realEstateCommission: s.realEstateCommission?.toString() || "",
        // Bonificações
        possuiBonificacao: s.possuiBonificacao || false,
        tipoBonificacao: s.tipoBonificacao || "",
        valorBonificacao: s.valorBonificacao?.toString() || "",
        descricaoBonificacao: s.descricaoBonificacao || "",
        comissaoBonificacaoCorretor: s.comissaoBonificacaoCorretor?.toString() || "",
        comissaoBonificacaoImobiliaria: s.comissaoBonificacaoImobiliaria?.toString() || "",
        // Sinal de Negócio
        sinalNegocio: s.sinalNegocio || "",
        sinalNegocioEmpresa: s.sinalNegocioEmpresa || "",
        sinalNegocioValor: s.sinalNegocioValor?.toString() || "",
        sinalNegocioDataPagamento: s.sinalNegocioDataPagamento ? new Date(s.sinalNegocioDataPagamento).toISOString().split('T')[0] : "",
        sinalNegocioComprovanteUrl: s.sinalNegocioComprovanteUrl || "",
        // Observações
        observations: s.observations || s.observation || "",
      }));
    }
  ...
  useEffect(() => {
    // Todos os campos do formulário para mostrar verde quando preenchidos
    const newCompletionStatus: CompletionStatus = {};
    const allFields = Object.keys(formData) as (keyof FormData)[];
    allFields.forEach((field) => {
      const value = formData[field];
      if (Array.isArray(value)) {
        // Arrays (buyers, sellers): não marca como preenchido baseado no array em si
        newCompletionStatus[field as string] = false;
      } else if (typeof value === 'string') {
        newCompletionStatus[field as string] = value !== "" && value !== "Selecione";
      } else if (typeof value === 'boolean') {
        newCompletionStatus[field as string] = value;
      } else {
        newCompletionStatus[field as string] = value !== null && value !== undefined;
      }
    });
    // Sincronizar buyerName e sellerName com o primeiro item dos arrays
    const firstBuyerName = formData.buyers[0]?.name || formData.buyerName || "";
    const firstSellerName = formData.sellers[0]?.name || formData.sellerName || "";
    newCompletionStatus['buyerName'] = firstBuyerName !== "";
    newCompletionStatus['sellerName'] = firstSellerName !== "";
    setCompletionStatus((prev) => ({ ...prev, ...newCompletionStatus }));
  }, [formData]);
  ...
  // Mapa de nomes amigáveis para exibição na mensagem de erro
  const fieldLabels: Record<string, string> = {
    ...
    sinalNegocio: "Sinal de Negócio",
    sinalNegocioValor: "Valor do Sinal de Negócio",
    sinalNegocioDataPagamento: "Data do Pagamento do Sinal",
  };
  ...
  const handleSaveDraft = async () => {
    try {
      ...
      // Validar datas com segurança
      const getSafeDate = (dateStr: string | null): Date | null => {
        if (!dateStr) return null;
        const d = new Date(dateStr);
        return isNaN(d.getTime()) ? null : d;
      };
      
      const saleDateObj = getSafeDate(formData.saleDate) || new Date();
      const angariationDateObj = getSafeDate(formData.angariationDate);
      const expectedPaymentDateObj = getSafeDate(formData.expectedPaymentDate);
      
      const payload = {
        propertyType: (formData.propertyType === "baggio" || formData.propertyType === "external" ? formData.propertyType : "baggio") as "baggio" | "external",
        ...
        angariationDate: angariationDateObj && !isNaN(angariationDateObj.getTime()) ? angariationDateObj.toISOString() : undefined,
        expectedPaymentDate: expectedPaymentDateObj && !isNaN(expectedPaymentDateObj.getTime()) ? expectedPaymentDateObj.toISOString() : undefined,
        saleValue: formData.saleValue ? parseCurrencyInput(formData.saleValue) : 0,
        buyerName: formData.buyers[0]?.name || formData.buyerName || "",
        buyerCpfCnpj: (formData.buyers[0]?.cpfCnpj || formData.buyerCpfCnpj || "").replace(/\D/g, ""),
        buyerPhone: (formData.buyers[0]?.phone || formData.buyerPhone || "").replace(/\D/g, ""),
        clientOrigin: formData.buyers[0]?.origin || formData.clientOrigin,
        paymentMethod: formData.paymentMethod,
        financedValue: formData.financedValue ? parseFloat(formData.financedValue) : undefined,
        sellerName: formData.sellers[0]?.name || formData.sellerName,
        sellerCpfCnpj: (formData.sellers[0]?.cpfCnpj || formData.sellerCpfCnpj || "").replace(/\D/g, ""),
        sellerPhone: (formData.sellers[0]?.phone || formData.sellerPhone || "").replace(/\D/g, ""),
        cartoryBank: formData.cartoryBank || undefined,
        despachante: formData.despachante || undefined,
        ...
        // Comissões: converter para número, evitar NaN
        totalCommission: formData.totalCommissionValue && !isNaN(parseFloat(formData.totalCommissionValue)) ? parseFloat(formData.totalCommissionValue) : (commissionCalc?.totalCommissionValue || 0),
        totalCommissionPercent: formData.totalCommissionPercent && !isNaN(parseFloat(formData.totalCommissionPercent)) ? parseFloat(formData.totalCommissionPercent) : (commissionCalc?.totalCommissionPercent || 0),
        angariadorCommission: formData.angariadorCommission && !isNaN(parseFloat(formData.angariadorCommission)) ? parseFloat(formData.angariadorCommission) : (commissionCalc?.angariadorValue || 0),
        vendedorCommission: formData.vendedorCommission && !isNaN(parseFloat(formData.vendedorCommission)) ? parseFloat(formData.vendedorCommission) : (commissionCalc?.vendedorValue || 0),
        realEstateCommission: formData.realEstateCommission && !isNaN(parseFloat(formData.realEstateCommission)) ? parseFloat(formData.realEstateCommission) : undefined,
        baggioCommission: commissionCalc?.baggioValue || 0,
        observations: formData.observations,
        typeOfProperty: formData.typeOfProperty,
        bedrooms: formData.bedrooms === "Stúdio" ? 0 : (formData.bedrooms && !isNaN(parseInt(formData.bedrooms)) ? parseInt(formData.bedrooms) : undefined),
        ...
        // Campos adicionais que estavam faltando no rascunho
        ...
        bankReturnPercentage: formData.bankReturnPercentage ? parseFloat(formData.bankReturnPercentage as string) : undefined,
        downPaymentPercentage: formData.downPaymentPercentage ? parseFloat(formData.downPaymentPercentage as string) : undefined,
        contractNumber: formData.contractNumber as string || undefined,
        portfolioStatus: formData.portfolioStatus as string || undefined,
        deedStatus: formData.deedStatus as string || undefined,
        managementResponsible: formData.managementResponsible as string || undefined,
        listingStore: formData.listingStore as string || undefined,
        sellingStore: formData.sellingStore as string || undefined,
        team: formData.team as string || undefined,
        region: formData.region as string || undefined,
        // Sistema de Comissionamento Automático
        tipoComissao: formData.tipoComissao || undefined,
        porcentagemComissao: formData.porcentagemComissao || undefined,
        comissaoTotal: formData.comissaoTotal || undefined,
        comissaoCoordenador: formData.comissaoCoordenador || undefined,
        comissaoAngariador: formData.comissaoAngariador || undefined,
        comissaoVendedor: formData.comissaoVendedor || undefined,
        comissaoImobiliaria: formData.comissaoImobiliaria || undefined,
        comissaoParceira: formData.comissaoParceira || undefined,
        comissaoAutonomo: formData.comissaoAutonomo || undefined,
        // Bonificações
        possuiBonificacao: formData.possuiBonificacao || false,
        tipoBonificacao: formData.tipoBonificacao || undefined,
        valorBonificacao: formData.valorBonificacao || undefined,
        descricaoBonificacao: formData.descricaoBonificacao || undefined,
        comissaoBonificacaoCorretor: formData.comissaoBonificacaoCorretor || undefined,
        comissaoBonificacaoImobiliaria: formData.comissaoBonificacaoImobiliaria || undefined,
        // Sinal de Negócio
        sinalNegocio: (formData.sinalNegocio as "Baggio" | "Outra") || undefined,
        sinalNegocioEmpresa: formData.sinalNegocioEmpresa || undefined,
        sinalNegocioValor: formData.sinalNegocioValor ? parseFloat(formData.sinalNegocioValor) : undefined,
      ...
      // Se está em modo edição, usar UPDATE ao invés de CREATE
      let savedSaleId: string | undefined = editId || undefined;
      if (isEditMode && editId) {
        ...
        savedSaleId = result?.saleId;
        toast.success("Rascunho salvo com sucesso!");
      }

      // Upload do arquivo de proposta se houver
      if (proposalFile && savedSaleId) {
        try {
          ...
          console.error('Erro ao fazer upload do documento no rascunho:', uploadError);
        }
      }

      // Upload do comprovante do sinal de negócio se houver
      if (formData.sinalNegocioComprovanteFile && savedSaleId) {
        try {
          const base64Sinal = await new Promise<string>((resolve, reject) => {
            const reader = new FileReader();
            reader.onload = () => {
              const r = reader.result as string;
              resolve(r.split(',')[1]);
          ...
          const dataUrl = `data:${formData.sinalNegocioComprovanteFile!.type};base64,${base64Sinal}`;
          await updateSaleMutation.mutateAsync({
            saleId: savedSaleId,
            propertyType: (formData.propertyType === "baggio" || formData.propertyType === "external") ? formData.propertyType : "baggio",
            saleDate: saleDateObj.toISOString(),
            businessType: formData.businessType || formData.tipoComissao || "sale",
            sinalNegocioComprovanteUrl: dataUrl
          });
        } catch (uploadError) {
          console.error('Erro ao fazer upload do comprovante de sinal no rascunho:', uploadError);
        }
      }
  ...
  const handleSubmitProposal = async () => {
    ...
    if (!proposalFile && !existingDocUrl?.url && !existingSale?.proposalDocumentUrl) {
    ...
    if (!isFormComplete) {
      ...
      toast.error(`Preencha todos os campos obrigatórios: ${missingLabels.join(', ')}`);
  ...
  const handleConfirmAndSave = async () => {
    try {
      ...
      const payload = {
        ...
        propertyComplement: formData.propertyComplement || undefined,
        propertyNeighborhood: formData.propertyNeighborhood || undefined,
        ...
        saleValue: parseCurrencyInput(formData.saleValue),
        buyerName: formData.buyers[0]?.name || formData.buyerName || "",
        buyerCpfCnpj: (formData.buyers[0]?.cpfCnpj || formData.buyerCpfCnpj || "").replace(/\D/g, ""),
        buyerPhone: (formData.buyers[0]?.phone || formData.buyerPhone || "").replace(/\D/g, ""),
        clientOrigin: formData.buyers[0]?.origin || formData.clientOrigin,
        ...
        sinalNegocioDataPagamento: formData.sinalNegocioDataPagamento ? (() => {
          const date = new Date(formData.sinalNegocioDataPagamento);
      ...
      if (isEditMode && editId) {
        await updateSaleMutation.mutateAsync({ saleId: editId, ...payload });
        result = { saleId: editId };
      } else {
        result = await createSaleMutation.mutateAsync(payload);
      }
      ...
      if (formData.sinalNegocioComprovanteFile && result.saleId) {
        try {
          ...
          // Salvar a imagem como data URL (base64) no campo sinalNegocioComprovanteUrl
          const dataUrl = `data:${formData.sinalNegocioComprovanteFile!.type};base64,${base64}`;
          
          // Atualizar a venda com a URL do comprovante
          await updateSaleMutation.mutateAsync({
            saleId: result.saleId,
            propertyType: (formData.propertyType === "baggio" || formData.propertyType === "external") ? formData.propertyType : "baggio",
            saleDate: (getSafeDate(formData.saleDate) || new Date()).toISOString(),
            businessType: formData.businessType || formData.tipoComissao || "sale",
            sinalNegocioComprovanteUrl: dataUrl
          });
          
          console.log('[DEBUG] Comprovante de sinal salvo com sucesso');
        } catch (uploadError) {
            ...
            <CardContent className="space-y-6">
              ...
              <div className="border-b pb-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    <p className="text-muted-foreground">Área Privativa</p>
                  ...
                  <div>
                    <p className="text-muted-foreground">Valor de Divulgação</p>
              ...
              <div className="border-b pb-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    <p className="text-muted-foreground">Data da Angariação</p>
                  ...
                  <div>
                    ...
                    <p className="font-medium text-lg text-green-700">{formatCurrency(parseCurrencyInput(formData.saleValue))}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Previsão de Recebimento</p>
                  ...
                  <div className={completionStatus.businessType ? "bg-green-50 p-2 rounded" : attemptedSave && !completionStatus.businessType ? "bg-red-50 p-2 rounded border border-red-300" : ""}>
              ...
              {(commissionCalc || formData.comissaoTotal) && (
                <div className="border-b pb-4 bg-blue-50 p-4 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="col-span-2">
                      <p className="text-muted-foreground">Total da Comissão Fechada</p>
                      <p className="font-bold text-xl text-blue-700">
                        {formatCurrency(commissionCalc?.totalCommissionValue || parseFloat(formData.comissaoTotal || "0") || 0)}
                        {" "}({commissionCalc?.totalCommissionPercent || formData.porcentagemComissao || "0"}%)
                      </p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Comissão Corretor Angariador ({commissionCalc?.angariadorPercent || formData.comissaoAngariadorPerc || "0"}%)</p>
                      <p className="font-medium text-green-700">{formatCurrency(commissionCalc?.angariadorValue || parseFloat(formData.comissaoAngariador || "0") || 0)}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Comissão Corretor Vendedor ({commissionCalc?.vendedorPercent || formData.comissaoVendedorPerc || "0"}%)</p>
                      <p className="font-medium text-green-700">{formatCurrency(commissionCalc?.vendedorValue || parseFloat(formData.comissaoVendedor || "0") || 0)}</p>
                    </div>
                    <div className="col-span-2">
                      <p className="text-muted-foreground">Comissão Baggio ({commissionCalc?.baggioPercent || "0"}%)</p>
                      <p className="font-medium text-foreground">{formatCurrency(commissionCalc?.baggioValue || parseFloat(formData.comissaoImobiliaria || "0") || 0)}</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="border-b pb-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className={completionStatus.buyerName ? "bg-green-50 p-2 rounded" : attemptedSave && !completionStatus.buyerName ? "bg-red-50 p-2 rounded border border-red-300" : ""}>
                    <p className="text-muted-foreground">Nome</p>
                    <p className="font-medium">{formData.buyerName}</p>
                  </div>
                  <div className={completionStatus.buyerCpfCnpj ? "bg-green-50 p-2 rounded" : ""}>
                  ...
                  <div>
                    <p className="text-muted-foreground">Origem do Cliente</p>
              ...
              <div className="border-b pb-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className={completionStatus.sellerName ? "bg-green-50 p-2 rounded" : attemptedSave && !completionStatus.sellerName ? "bg-red-50 p-2 rounded border border-red-300" : ""}>
                    <p className="text-muted-foreground">Nome</p>
                    <p className="font-medium">{formData.sellerName}</p>
                  </div>
                  <div className={completionStatus.sellerCpfCnpj ? "bg-green-50 p-2 rounded" : ""}>
                    <p className="text-muted-foreground">CPF/CNPJ</p>
              ...
              <div className="border-b pb-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <p className="text-muted-foreground">Corretor Angariador</p>
                  ...
                  <div>
                    <p className="text-muted-foreground">Comissão Angariador</p>
                    <p className="font-medium">{formData.angariadorCommission ? formatCurrency(parseFloat(formData.angariadorCommission)) : formData.comissaoAngariador ? formatCurrency(parseFloat(formData.comissaoAngariador)) : "-"}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Corretor Vendedor</p>
                    <p className="font-medium">
                  ...
                  <div>
                    <p className="text-muted-foreground">Comissão Vendedor</p>
                    <p className="font-medium">{formData.vendedorCommission ? formatCurrency(parseFloat(formData.vendedorCommission)) : formData.comissaoVendedor ? formatCurrency(parseFloat(formData.comissaoVendedor)) : "-"}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Comissão Imobiliária</p>
                    <p className="font-medium">{formData.realEstateCommission ? formatCurrency(parseFloat(formData.realEstateCommission)) : formData.comissaoImobiliaria ? formatCurrency(parseFloat(formData.comissaoImobiliaria)) : "-"}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 px-4 py-2 bg-muted text-foreground rounded-lg hover:bg-slate-300"
...
{/* Título da Página */}
            <div className="mb-6">
              <h1 className="text-2xl font-bold text-foreground">{isEditMode ? "Editar Venda" : "Nova Venda"}</h1>
              <p className="text-muted-foreground mt-1 text-sm">{isEditMode ? "Atualize os dados da venda" : "Preencha os campos para registrar uma nova venda"}</p>
            </div>
          ...
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex gap-2">
                  <div className="flex-1 relative">
                    <Input
                      ...
                      className={`pr-10 ${completionStatus.propertyReference ? "bg-green-50 border-green-300" : ""}`}
                    />
                ...
                <p className="text-xs text-muted-foreground mt-1">Digite o código de referência do imóvel (ex: BG97142005, BG96925001)</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Select value={formData.typeOfProperty} onValueChange={(value) => handleInputChange("typeOfProperty", value)}>
                      <SelectTrigger style={getProperfyFieldStyle("typeOfProperty", formData.typeOfProperty, completionStatus.typeOfProperty, attemptedSave, true)}>
                  ...
                  <div>
                    ...
                    >
                      <SelectTrigger style={getProperfyFieldStyle("bedrooms", formData.bedrooms, completionStatus.bedrooms, attemptedSave, true)}>
                        <SelectValue placeholder="Selecione" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="Stúdio">Stúdio</SelectItem>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      style={getProperfyFieldStyle("privateArea", formData.privateArea, completionStatus.privateArea, attemptedSave, true)}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      style={getProperfyFieldStyle("totalArea", formData.totalArea, completionStatus.totalArea, attemptedSave, true)}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      style={getProperfyFieldStyle("costPerM2", formData.costPerM2)}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      style={getProperfyFieldStyle("propertyAge", formData.propertyAge)}
                    />
                  </div>
                </div>
              </CardContent>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      maxLength={9}
                      style={getProperfyFieldStyle("propertyZipCode", formData.propertyZipCode)}
                    />
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      style={getProperfyFieldStyle("condominiumName", formData.condominiumName)}
                    />
                  </div>
                  <div className="col-span-2">
                    ...
                    <Input
                      ...
                      style={getProperfyFieldStyle("propertyAddress", formData.propertyAddress, completionStatus.propertyAddress, attemptedSave, true)}
                    />
                  </div>
                  <div>
                    <Label>Número</Label>
                    <Input
                      ...
                      style={getProperfyFieldStyle("propertyNumber", formData.propertyNumber)}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      className={completionStatus.propertyComplement ? "bg-green-50 border-green-300" : ""}
                    />
                  </div>
                  <div>
                    <Label>Bairro</Label>
                    <Input
                      ...
                      style={getProperfyFieldStyle("propertyNeighborhood", formData.propertyNeighborhood)}
                    />
                  </div>
                  <div>
                    <Label>Cidade</Label>
                    <Input
                      ...
                      style={getProperfyFieldStyle("propertyCity", formData.propertyCity)}
                    />
                  </div>
                  <div>
                    <Label>Estado *</Label>
                    <Select value={formData.propertyState} onValueChange={(value) => handleInputChange("propertyState", value)}>
                      <SelectTrigger style={getProperfyFieldStyle("propertyState", formData.propertyState)}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      style={getProperfyFieldStyle("saleDate", formData.saleDate, completionStatus.saleDate, attemptedSave, true)}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      style={getProperfyFieldStyle("angariationDate", formData.angariationDate)}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="R$ 0,00"
                      style={getProperfyFieldStyle("saleValue", formData.saleValue, completionStatus.saleValue, attemptedSave, true)}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="0,00"
                      style={getProperfyFieldStyle("advertisementValue", formData.advertisementValue)}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      style={getProperfyFieldStyle("expectedPaymentDate", formData.expectedPaymentDate)}
                    />
                  </div>
                  <div>
                    <Label>Forma de Pagamento</Label>
                    <Select value={formData.paymentMethod} onValueChange={(value) => handleInputChange("paymentMethod", value)}>
                      <SelectTrigger className={completionStatus.paymentMethod ? "bg-green-50 border-green-300" : ""}>
                        <SelectValue placeholder="Selecione" />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      style={getProperfyFieldStyle("financedValue", formData.financedValue)}
                      placeholder="R$ 0,00"
                    />
                  </div>
                  <div>
                    ...
                    <Select value={formData.investmentType} onValueChange={(value) => handleInputChange("investmentType", value)}>
                      <SelectTrigger className={completionStatus.investmentType ? "bg-green-50 border-green-300" : ""}>
                        <SelectValue placeholder="Selecione" />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">Lançamento: Lucas | Pronto: Camila</p>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    {formData.sinalNegocio === "Baggio" && (
                      <div>
                        ...
                        <Input
                          ...
                          onChange={handleSinalComprovanteChange}
                        ...
                        />
                        <p className="text-xs text-muted-foreground mt-1">Máximo 5MB. Formatos aceitos: PDF, JPG, PNG</p>
                      </div>
                    )}
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        style={getProperfyFieldStyle("sinalNegocioValor", formData.sinalNegocioValor, completionStatus.sinalNegocioValor, attemptedSave, true)}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        style={getProperfyFieldStyle("sinalNegocioDataPagamento", formData.sinalNegocioDataPagamento, completionStatus.sinalNegocioDataPagamento, attemptedSave, true)}
                      />
                    </div>
                  </div>
                </div>
            ...
            <CommissionSection formData={formData} handleInputChange={handleInputChange} completionStatus={completionStatus} attemptedSave={attemptedSave} />
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                {/* Corretor Angariador */}
                <div className="border-t pt-4 mt-4">
                  <h4 className="font-semibold text-foreground mb-3">Corretor Angariador</h4>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <>
                        <div>
                          ...
                          <Input
                            ...
                            className={completionStatus.brokerAngariadorName ? "bg-green-50 border-green-300" : ""}
                          />
                        </div>
                        <div>
                          <Label>CRECI</Label>
                          <Input
                            ...
                            className={completionStatus.brokerAngariadorCreci ? "bg-green-50 border-green-300" : ""}
                          />
                        </div>
                        <div>
                          <Label>Email</Label>
                          <Input
                            ...
                            className={completionStatus.brokerAngariadorEmail ? "bg-green-50 border-green-300" : ""}
                          />
                        </div>
                        <div>
                          ...
                          <Input
                            ...
                            value={formData.brokerAngariadorRealEstateCompany || ""}
                            onChange={(e) => handleInputChange("brokerAngariadorRealEstateCompany", e.target.value)}
                            className={completionStatus.brokerAngariadorRealEstateCompany ? "bg-green-50 border-green-300" : ""}
                          />
                        </div>
                        <div>
                          ...
                          <Input
                            ...
                            value={formData.brokerAngariadorRealEstateCnpj || ""}
                            onChange={(e) => handleInputChange("brokerAngariadorRealEstateCnpj", e.target.value)}
                            className={completionStatus.brokerAngariadorRealEstateCnpj ? "bg-green-50 border-green-300" : ""}
                          />
                        </div>
                      </>
                    )}
                ...
                {/* Corretor Vendedor */}
                <div className="border-t pt-4 mt-4">
                  <h4 className="font-semibold text-foreground mb-3">Corretor Vendedor</h4>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label>Tipo de Corretor *</Label>
                      ...
                      <>
                        <div>
                          ...
                          <Input
                            ...
                            className={completionStatus.brokerVendedorName ? "bg-green-50 border-green-300" : ""}
                          />
                        </div>
                        <div>
                          <Label>CRECI</Label>
                          <Input
                            ...
                            className={completionStatus.brokerVendedorCreci ? "bg-green-50 border-green-300" : ""}
                          />
                        </div>
                        <div>
                          <Label>Email *</Label>
                          <Input
                            ...
                            className={completionStatus.brokerVendedorEmail ? "bg-green-50 border-green-300" : ""}
                          />
                        </div>
                      </>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input
                          placeholder="Nome completo"
                          value={seller.name}
                          style={seller.name ? { backgroundColor: '#dcfce7', borderColor: '#86efac' } : undefined}
                          onChange={(e) => setFormData(prev => ({
                            ...prev,
                            sellers: prev.sellers.map((s, i) => i === index ? { ...s, name: e.target.value } : s),
                            sellerName: index === 0 ? e.target.value : prev.sellerName
                          }))}
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          ...
                          style={seller.cpfCnpj ? { backgroundColor: '#dcfce7', borderColor: '#86efac' } : undefined}
                          onChange={(e) => setFormData(prev => ({
                            ...prev,
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          value={seller.phone}
                          style={seller.phone ? { backgroundColor: '#dcfce7', borderColor: '#86efac' } : undefined}
                          onChange={(e) => setFormData(prev => ({
                            ...prev,
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input
                          placeholder="Nome completo"
                          value={buyer.name}
                          style={buyer.name ? { backgroundColor: '#dcfce7', borderColor: '#86efac' } : undefined}
                          onChange={(e) => setFormData(prev => ({
                            ...prev,
                            buyers: prev.buyers.map((b, i) => i === index ? { ...b, name: e.target.value } : b),
                            buyerName: index === 0 ? e.target.value : prev.buyerName
                          }))}
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          placeholder="000.000.000-00"
                          value={buyer.cpfCnpj}
                          style={buyer.cpfCnpj ? { backgroundColor: '#dcfce7', borderColor: '#86efac' } : undefined}
                          onChange={(e) => setFormData(prev => ({
                            ...prev,
                      ...
                      <div>
                        ...
                        <Input
                          placeholder="(00) 00000-0000"
                          value={buyer.phone}
                          style={buyer.phone ? { backgroundColor: '#dcfce7', borderColor: '#86efac' } : undefined}
                          onChange={(e) => setFormData(prev => ({
                            ...prev,
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      style={getProperfyFieldStyle("cartoryBank", formData.cartoryBank)}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      style={getProperfyFieldStyle("despachante", formData.despachante)}
                    />
                  </div>
                </div>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                {/* Mostrar arquivo já enviado no modo edição */}
                {isEditMode && !proposalFile && (existingDocUrl?.url || existingSale?.proposalDocumentUrl) && (
                  <div className="mb-3 flex items-center gap-3 px-4 py-3 bg-green-50 border border-green-200 rounded-lg">
                    <FileText className="h-5 w-5 text-green-600 flex-shrink-0" />
                    <span className="text-green-700 font-medium text-sm flex-1">
                      {(existingDocUrl?.url || existingSale?.proposalDocumentUrl || "").split('/').pop() || 'Documento enviado'}
                    </span>
                    <Button
                      type="button"
                      size="sm"
                      ...
                      onClick={() => window.open(existingDocUrl?.url || existingSale?.proposalDocumentUrl || "", "_blank")}
                    >
                ...
                <div className="flex items-center gap-4">
                  <label className="flex-1 flex items-center justify-center gap-2 px-4 py-8 border-2 border-dashed border-border rounded-lg cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-colors">
                    ...
                    {proposalFile ? (
                      ...
                      <>
                        ...
                        <span className="text-muted-foreground">
                          {isEditMode && (existingDocUrl?.url || existingSale?.proposalDocumentUrl)
                            ? 'Clique para substituir o documento atual'
                            : 'Clique para anexar documento de venda (PDF, DOC, JPG)'}
                        </span>
                      </>
                    )}
                  </label>
                </div>
                <p className="text-xs text-muted-foreground mt-2">Máximo 10MB. O arquivo será enviado após salvar a venda.</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <Textarea
                  ...
                  rows={4}
                  style={getProperfyFieldStyle("observations", formData.observations)}
                />
              </CardContent>
            </Card>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="px-4 py-2 bg-muted text-foreground rounded-lg hover:bg-slate-300 font-medium"
              >
                Cancelar
              </button>
              <button
                ...
                onMouseEnter={(e) => !isSubmitting && (e.currentTarget.style.backgroundColor = '#ea580c')}
