...
Mantém exatamente igual às planilhas e vincula corretores
"""
...
# Mapeamento de nomes de corretores para IDs (baseado em corretores já cadastrados)
BROKER_MAPPING = {
...
def load_broker_mapping(connection):
    """Carrega mapeamento de nomes de corretores para IDs"""
    cursor = connection.cursor(dictionary=True)
    cursor.execute("SELECT id, name FROM brokers WHERE companyId = %s", (os.getenv("COMPANY_ID", "B I IMOVEIS LTDA"),))
...
def normalize_broker_name(name):
    """Normaliza nome de corretor para busca"""
    if not name:
        return None
...
def import_monthly_file(connection, file_path, month_num, year=2025):
    ...
    try:
        ...
        for row_idx, row in enumerate(data_rows, start=4):
            try:
                ...
                # Buscar IDs de corretores
                angariador_id = None
                if angariador_name:
                    normalized = normalize_broker_name(angariador_name)
                    angariador_id = BROKER_MAPPING.get(normalized)
                ...
                values = (
                    ...
                    property_name or "N/A",  # buyerName (usando nome do imóvel como fallback)
                    sale_date,
                    angaria_date,
                    sale_value,
                    angariador_id,
                ...
                if imported % 10 == 0:
                    print(f"  ✓ {imported} linhas processadas...")
...
def main():
    ...
    for month_num in range(1, 13):
        ...
        # Procurar arquivo
        file_pattern = f"{month_str} Relatório_{month_name}_2025"
        file_path = None
        ...
        if file_path and os.path.exists(file_path):
            imported, errors = import_monthly_file(connection, file_path, month_num)
    ...
    print(f"Taxa de sucesso: {(total_imported / (total_imported + total_errors) * 100):.1f}%" if (total_imported + total_errors) > 0 else "N/A")
